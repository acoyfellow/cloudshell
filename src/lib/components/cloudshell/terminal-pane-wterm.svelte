<script lang="ts">
  /**
   * `terminal-pane-wterm.svelte` — spike alternative to terminal-pane.svelte
   * that uses `@wterm/dom` (Zig + WASM core + DOM renderer) instead of
   * xterm.js's canvas renderer.
   *
   * Why: xterm's canvas renderer makes Ctrl+F / browser find impossible,
   * text selection across wrapped rows janky, and a11y essentially broken.
   * wterm renders to real DOM spans, so everything the browser gives you
   * natively Just Works: find, select, zoom, accessibility tree, etc.
   *
   * The reconnect / ticket-delivery / control-message / bridge plumbing
   * is copied verbatim from terminal-pane.svelte. Only the terminal
   * library is different. When the spike proves out, this replaces the
   * xterm version on main.
   *
   * Activation: ?terminal=wterm query param (see utility-pane.svelte).
   */
