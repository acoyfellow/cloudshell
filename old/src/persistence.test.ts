...
describe('Per-user directory isolation', () => {
  ...
  describe('getUserSessionContainerId', () => {
    it('should append a normalized session id to the user container id', async () => {
      const { getUserSessionContainerId } = await import('./auth');

      expect(getUserSessionContainerId('alice', 'main')).toBe('shell:alice:main');
      expect(getUserSessionContainerId('alice', 'Review_01')).toBe('shell:alice:review-01');
    });
  });
});
