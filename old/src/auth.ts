...
function getJwtSecret(env?: { JWT_SECRET?: string }): string {
  ...
  if (!secret) {
    throw new Error(
      'JWT_SECRET environment variable is required. Set it via: wrangler secret put JWT_SECRET'
    );
  }
  return secret;
}
...
export function getUserContainerId(username: string): string {
  return `shell:${normalizeContainerSegment(username)}`;
}

export function getUserSessionContainerId(username: string, sessionId: string): string {
  return `${getUserContainerId(username)}:${normalizeContainerSegment(sessionId)}`;
}
